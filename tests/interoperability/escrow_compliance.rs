...
use crate::interoperability::mocks::{
    MockTokenClient, MockKYCRegistryClient, MockSanctionsClient, MockVelocityLimitsClient
};
use mentorminds_escrow::{EscrowContract, EscrowContractClient, EscrowParams};

fn setup_env<'a>(env: &'a Env) -> (Address, Address, EscrowContractClient<'a>, MockTokenClient<'a>, MockKYCRegistryClient<'a>, MockSanctionsClient<'a>, MockVelocityLimitsClient<'a>) {
    env.mock_all_auths();
    ...
    let vel_id = env.register_contract(None, crate::interoperability::mocks::MockVelocityLimits);
    ...
    (admin, treasury, escrow_client, token_client, kyc_client, sanc_client, vel_client)
}
...
fn test_failure_velocity_limit() {
    let env = Env::default();
    let (_, _, escrow_client, token_client, kyc_client, sanc_client, vel_client) = setup_env(&env);
    let learner = Address::generate(&env);
    let mentor = Address::generate(&env);
